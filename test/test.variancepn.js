...
tape( 'the function has an arity of 4', function test( t ) {
	t.strictEqual( variancepn.length, 4, 'has expected arity' );
	t.end();
});
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = variancepn( 0, 1, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = variancepn( -1, 1, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a `correction` parameter yielding `N-correction` less than or equal to `0`, the function returns `NaN`', function test( t ) {
	...
	v = variancepn( x.length, x.length, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = variancepn( x.length, x.length+1, x, 1 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
